...
{
    public class FrmGererCategories : Form
    {
        private readonly Form1? _mainForm;

        public int? DerniereCategorieModifieeId { get; private set; } = null;
        ...
        private Guna2DataGridView tableCategoriesDataGridView = null!;
        ...
        private void ConstruireControles()
        {
            ...
            foreach (var (affichage, _) in ColonnesFiltrablesCategorie)
            ...
            // Bouton Ajouter
            btnNouvelleCategorie = new Guna2Button
            {
            ...
            rightActionsPanel.Controls.Add(btnNouvelleCategorie);
            ...
            // 2. DataGridView
            tableCategoriesDataGridView = new Guna2DataGridView
            {
                Dock = DockStyle.Fill,
            ...
            tableCategoriesDataGridView.RowTemplate.Height = 38;

            tableCategoriesDataGridView.AlternatingRowsDefaultCellStyle = new DataGridViewCellStyle
            {
                BackColor = Color.White,
            ...
            tableCategoriesDataGridView.ColumnHeadersDefaultCellStyle = new DataGridViewCellStyle
            {
            ...
            tableCategoriesDataGridView.ColumnHeadersHeight = 40;

            tableCategoriesDataGridView.DefaultCellStyle = new DataGridViewCellStyle
            {
                Alignment = DataGridViewContentAlignment.MiddleLeft,
                BackColor = Color.White,
                Font = new Font("Segoe UI", 9F, FontStyle.Bold),
            ...
            tableCategoriesDataGridView.CellPainting += Dgv_CellPainting;
            tableCategoriesDataGridView.CellClick += Dgv_CellClick;
            tableCategoriesDataGridView.CellMouseMove += (s, e) => tableCategoriesDataGridView.InvalidateCell(e.ColumnIndex, e.RowIndex);
            tableCategoriesDataGridView.CellMouseLeave += (s, e) => tableCategoriesDataGridView.InvalidateCell(e.ColumnIndex, e.RowIndex);

            Controls.Add(tableCategoriesDataGridView);
        ...
        {
            if (tableCategoriesDataGridView.DataSource is not DataTable dt) return;
            ...
            else
            {
                ...
                string colonne = (index >= 0 && index < ColonnesFiltrablesCategorie.Length) ? ColonnesFiltrablesCategorie[index].Colonne : "";
        ...
        private void BtnNouvelleCategorie_Click(object? sender, EventArgs e)
        {
            using (var frm = new FrmAjouterCategorie())
            {
                ...
                {
                    DerniereCategorieModifieeId = frm.CategorieIdResultat;
                    ChargerListe();
                }
            }
        }
        ...
        private void BtnChoisirColonnes_Click(object? sender, EventArgs e)
        {
            ...
            foreach (DataGridViewColumn col in tableCategoriesDataGridView.Columns)
            {
        ...
        private void BtnImprimer_Click(object? sender, EventArgs e)
        {
            ...
            html.Append("<h1>Liste des Catégories</h1><table><tr>");
            ...
            string tempFile = Path.Combine(Path.GetTempPath(), $"rapport_categories_{DateTime.Now:yyyyMMdd_HHmmss}.html");
        ...
        private void Dgv_CellPainting(object? sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            Point mousePos = tableCategoriesDataGridView.PointToClient(Cursor.Position);
            bool isHovered = e.CellBounds.Contains(mousePos);
            bool isClicked = isHovered && (Control.MouseButtons == MouseButtons.Left);

            if (e.ColumnIndex == tableCategoriesDataGridView.Columns["colModifier"]?.Index)
        ...
        {
            if (e.RowIndex < 0) return;
            string colName = tableCategoriesDataGridView.Columns[e.ColumnIndex].Name;
            int id = Convert.ToInt32(tableCategoriesDataGridView.Rows[e.RowIndex].Cells["ID"].Value);

            if (colName == "colModifier")
            {
                using (var frm = new FrmAjouterCategorie(id))
                {
                    if (frm.ShowDialog(this) == DialogResult.OK)
                    {
            ...
            {
                int nbModeles = Convert.ToInt32(tableCategoriesDataGridView.Rows[e.RowIndex].Cells["Modèles liés"].Value);
                string designation = tableCategoriesDataGridView.Rows[e.RowIndex].Cells["Désignation"].Value?.ToString() ?? "";

                if (nbModeles > 0)
                {
                    MessageBox.Show(
                        $"Impossible de supprimer '{designation}' : {nbModeles} modèle(s) sont encore associés à cette catégorie.\n\n" +
                        "Modifiez ou supprimez d'abord ces modèles.",
                        "Suppression refusée", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                var confirm = MessageBox.Show($"Supprimer définitivement la catégorie '{designation}' ?",
                    "Confirmer la suppression", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (confirm != DialogResult.Yes) return;

                try
                {
                    DatabaseHelper.ExecuteNonQuery("DELETE FROM Categorie WHERE id=@id", new SqliteParameter("@id", id));
                    ChargerListe();
                }
                catch (SqliteException ex) when (ex.SqliteErrorCode == 19)
                {
                    MessageBox.Show("Suppression refusée par la base de données : cette catégorie est encore utilisée.",
        ...
        private void ImprimerFicheIndividuelle(DataGridViewRow row)
        {
            ...
            html.Append("</style></head><body>");
            html.Append("<div class='title'>FICHE DE CATÉGORIE</div><table>");
            ...
            string tempFile = Path.Combine(Path.GetTempPath(), $"fiche_categorie_{row.Cells["ID"].Value}.html");
            File.WriteAllText(tempFile, html.ToString());
            Process.Start(new ProcessStartInfo(tempFile) { UseShellExecute = true });
        }
