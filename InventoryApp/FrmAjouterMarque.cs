...
namespace InventoryApp
{
    ...
    /// ou modifier UNE Marque. Reutilise par FrmAjouterArticle
    /// (creation rapide) et par FrmGererMarques (gestion complete).
    /// </summary>
    public class FrmAjouterMarque : Form
    {
        public int? MarqueIdResultat { get; private set; } = null;

        private readonly int? _marqueIdEnEdition;
        private bool EnModeEdition => _marqueIdEnEdition.HasValue;
        ...
        public FrmAjouterMarque(int? marqueIdEnEdition = null)
        {
            _marqueIdEnEdition = marqueIdEnEdition;
        ...
        private void ConstruireControles()
        {
            ...
            {
                Text = EnModeEdition ? "Modifier la marque" : "Nouvelle marque",
            ...
            txtCode = new Guna2TextBox { Left = 20, Top = 88, Width = 340, Height = 36, BorderRadius = 6, PlaceholderText = "Ex : HP, DEL, CAN" };

            var lblDesignation = new Label { Text = "Désignation *", Left = 20, Top = 132, Width = 300, ForeColor = Color.DimGray };
            txtDesignation = new Guna2TextBox { Left = 20, Top = 155, Width = 340, Height = 36, BorderRadius = 6, PlaceholderText = "Ex : Hewlett-Packard" };
        ...
        {
            var t = DatabaseHelper.ExecuteQuery("SELECT code, designation FROM Marque WHERE id=@id",
                new SqliteParameter("@id", _marqueIdEnEdition!.Value));
        ...
        private void BtnEnregistrer_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    if (EnModeEdition)
                    {
                        cmd.CommandText = "UPDATE Marque SET code=@code, designation=@desig WHERE id=@id";
                        cmd.Parameters.AddWithValue("@id", _marqueIdEnEdition!.Value);
                        cmd.Parameters.AddWithValue("@code", code);
                        cmd.Parameters.AddWithValue("@desig", desig);
                        cmd.ExecuteNonQuery();
                        MarqueIdResultat = _marqueIdEnEdition;
                    }
                    else
                    {
                        cmd.CommandText = "INSERT INTO Marque (code, designation) VALUES (@code, @desig); SELECT last_insert_rowid();";
                        cmd.Parameters.AddWithValue("@code", code);
                        cmd.Parameters.AddWithValue("@desig", desig);
                        MarqueIdResultat = Convert.ToInt32(cmd.ExecuteScalar());
                    }
                }
